...
#define TASK_MQTT_AWS
    ...
    #define AWS_MAX_MSG_SIZE_BYTES 512

    WiFiClientSecure AWS_net = WiFiClientSecure();
    ...
    void keepAWSConnectionAlive(void * parameter){
        for(;;){
            ...
            while (!AWS_mqtt.connect(DEVICE_NAME) && retries < 50) {
                Serial.print(".");
                delay(100);
                retries++;
            }
            ...
            //while (millis() - startAttemptTime < MQTT_CONNECT_TIMEOUT)
            //{
